...
export class CreationBudgetUseCase implements ICreationBudgetUseCase {
    ...
    async execute(request: CreationBudgetUseCaseRequest): Promise<void> {
        try {
            ...
            let period_time = null
            if (request.period_time <= 0 && request.is_periodic) {
            ...
            let period = null
            let date_to_update = null
            let date_start: DateParser = DateParser.from_string(request.date_start)
            if (is_empty(request.period) && request.is_periodic) {
                ...
                date_to_update = determined_end_date_with(date_start.toDate(), request.period, request.period_time)
            }

            let date_end = null 
            if (is_empty(request.date_end) && !request.is_periodic) {
                throw new ValidationError('Date end must')
            } else {
