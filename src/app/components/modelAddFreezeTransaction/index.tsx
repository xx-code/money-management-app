...
export default function ModalAddFreezeTrans ({isOpen, accounts, onClose, onAdd}: 
    {isOpen: boolean, accounts: AccountDisplay[], onClose: any, onAdd: any,} ) {

    const [input, setInput] = useState({account: '', price: 0, date: DateParser.now().toString()});
    const [errorInput, setErrorInput] = useState<{account: string|null,  price: string|null, date: string|null}>({account: null, price: null, date: null}) 
    ...
    async function submit() {
        try {
            ...
            if (do_submit) {
                let account = accounts.find(account => account.title === input.account);

                let new_transaction: RequestNewFreezeBalance = {
                    account_ref: account!.id,
                    price: input.price,
                    end_date: input.date
                };
            
                await axios.post('/api/transaction/freeze', new_transaction);
                await onAdd()
                closeModal();
            } else {
                // @ts-ignore
    ...
    function closeModal() {
        onClose();
        setErrorInput({account: null, price: null, date: null});
        setInput({account: '',  price: 0, date: DateParser.now().toString()});
    }
    ...
    return (
        <Modal isOpen={isOpen} onClose={() => {}}>
            <div className="modal-add-new-account">
                ...
                <TextInput type={'text'} title={'Account Envoyeur'} value={input.account} name={'account'} onChange={() => {}} options={accounts.map(account => account.title)} onClickOption={handleClickInput} error={errorInput.account} overOnBlur={undefined} />
                ...
                <div className="flex justify-around">
                    ...
                    <Button title="Freezer" onClick={submit} backgroundColor="#6755D7" colorText="white" />
                </div>
            </div>
        </Modal>
    )
